...
class User(UserMixin, db.Model):
    ...
    patreon_id = db.Column(db.String(200), unique=True, nullable=True)
    patreon_access_token = db.Column(db.String(500), nullable=True)
    patreon_refresh_token = db.Column(db.String(500), nullable=True)
    is_patron = db.Column(db.Boolean, default=False, nullable=False)
    patreon_tier_title = db.Column(db.String(200), nullable=True)
    patreon_pledge_cents = db.Column(db.Integer, nullable=True)
    patreon_linked_at = db.Column(db.DateTime, nullable=True)
    discord_id = db.Column(db.String(200), unique=True, nullable=True)
    discord_username = db.Column(db.String(100), nullable=True)
    discord_access_token = db.Column(db.String(500), nullable=True)
    discord_refresh_token = db.Column(db.String(500), nullable=True)
...
class LeagueWeek(db.Model):
    __tablename__ = "tracker_league_week"
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    league_id = db.Column(
        db.Integer, db.ForeignKey("tracker_league.id"), nullable=False
    )
...
class WeekMatchup(db.Model):
    __tablename__ = "tracker_week_matchup"
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    week_id = db.Column(
...
class PlayerMatchup(db.Model):
    ...
    is_double_loss = db.Column(db.Boolean, default=False, nullable=False)
...
class MatchGame(db.Model):
    __tablename__ = "tracker_match_game"
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    player_matchup_id = db.Column(
    ...
    loser_conceded = db.Column(db.Boolean, default=False, nullable=False)
    player1_deck_id = db.Column(
        db.Integer, db.ForeignKey("tracker_deck.id"), nullable=True
    )
    player2_deck_id = db.Column(
        db.Integer, db.ForeignKey("tracker_deck.id"), nullable=True
    )
...
class StrikeSelection(db.Model):
    __tablename__ = "tracker_strike_selection"
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    player_matchup_id = db.Column(
        db.Integer, db.ForeignKey("tracker_player_matchup.id"), nullable=False
    )
    striking_user_id = db.Column(
        db.Integer, db.ForeignKey("tracker_user.id"), nullable=False
    )
    struck_deck_selection_id = db.Column(
...
class ExtendedGameData(db.Model):
    ...
    turn_snapshots = db.Column(db.JSON, nullable=True)
    player2_turn_snapshots = db.Column(db.JSON, nullable=True)
    created_at = db.Column(
        db.DateTime, nullable=False, default=datetime.datetime.utcnow
    )
    updated_at = db.Column(
